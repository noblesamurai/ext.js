...
Ext.extend(String.prototype, {
  ...
  /**
   * Replaces _pattern_ with _replacement_. A more flexible alternative to String#replace.
   *
   * @param  {mixed} pattern
  ...
  gsub: function (pattern, replacement, scope) {
      if (typeof pattern === 'string' || pattern instanceof String)
        pattern = new RegExp(RegExp.escape(pattern), 'g')
      else
        pattern = new RegExp(pattern.source, 'g' + (pattern.multiline ? 'm' : '') +
                             (pattern.ignoreCase ? 'i' : ''))

      if (!(typeof replacement === 'string' || replacement instanceof String) &&
          !(replacement instanceof Function))
        replacement = replacement.toString()

      if (replacement instanceof Function)
        return this.replace(pattern, function () {
          return replacement.apply(scope || this, arguments)
        })
  ...
  /**
   * Retruens a Boolean value that indicates whether the current string includes the 
   * given string.
   *
  ...
  includes: function(s) { return this.indexOf(s) !== -1 },
  ...
  count: function(s) { return this.split(s).length - 1 }
})
